(()=>{
  const chunks=[{"src":"/verified-assets-01.js","from":1944,"to":1962,"ids":[64,63,80,81,82]}];
  const own=document.currentScript?.src||'';let v='';try{v=new URL(own,location.href).searchParams.get('v')||''}catch{}
  const loaded=new Map();
  function load(c){if(loaded.has(c.src))return loaded.get(c.src);const p=new Promise(resolve=>{const s=document.createElement('script');s.src=c.src+(v?'?v='+encodeURIComponent(v):'');s.async=true;s.onload=()=>resolve(true);s.onerror=()=>resolve(false);document.head.appendChild(s)});loaded.set(c.src,p);return p}
