...
const clock={baseAge:0,offset:0,lastRendered:null,lastTick:performance.now(),bank:0,armed:false,lastBoard:null,run:0};
// Scoren er alderen. Talet oppstraums rekna ut — 600 · fart · streak · alder —
// sa ingenting ein spelar kunne kjenne att, og stod i vegen for det som
// faktisk tel. I staden går klokka: kor lenge du har halde krona på hovudet.
const clockFmt=s=>{const t=Math.max(0,Math.floor(s)),h=Math.floor(t/3600),m=Math.floor(t%3600/60),x=t%60;
...
// Eldre rader ligg i basen med den gamle scoren i same felt. Eit tal som ikkje
// kan vere sekund i eit spel er heller ikkje det, så det blir ein strek.
const asTime=v=>v>0&&v<86400?clockFmt(v):'—';
function row(r){return `<li class="${r.isYou?'you':''}"><b>#${fmt.format(r.rank)}</b><strong>${r.age} år${r.isYou?'<i>DU</i>':''}</strong><span>${asTime(r.score)}</span><em>×${r.streak}</em></li>`}
function rowHead(){return '<div class="fh-cols"><b>PLASS</b><b>ALDER</b><b>TID</b><b>STREAK</b></div>'}
function renderBoard(data){const top=(data.top||[]).map(row).join('')||'<li class="empty">Ingen resultat enno</li>',near=(data.nearby||[]).map(row).join('');return `<header><h2>VERDSLISTE</h2><p>${fmt.format(data.total||0)} spelarar · alder først, tida ved lik alder</p></header><div class="fh-section"><h3>TOPP 10</h3>${rowHead()}<ol>${top}</ol></div>${near?`<div class="fh-gap">⋯</div><div class="fh-section fh-near"><h3>DIN PLASS <small>#${fmt.format(data.player.rank)}</small></h3>${rowHead()}<ol>${near}</ol></div>`:''}`}
...
function leafAny(texts){return [...document.querySelectorAll('*')].find(el=>el.children.length===0&&texts.includes(el.textContent?.trim()))}
// Runden er oppstraums si eiga folkemengd. crowd-assets.js tel henne opp kvar
// gong React byggjer eit nytt brett, og det er det einaste sikre teiknet på at
// eit trykk fann kongen — scoren står det ikkje lenger noko tal i.
function roundToken(){return document.documentElement.dataset.fhCrowdRound||''}
function scored(){return !!document.querySelector('.score-pop')}
function paintClock(){
  const label=leafAny(['SCORE','KLOKKA']);if(!label)return;
  if(label.textContent.trim()!=='KLOKKA')label.textContent='KLOKKA';
  // nb-NO skil tusen med hardt mellomrom, så «12 973» må telje som eit tal.
  const v=valueNearLabel(label,/^[\d.:\s\u00A0]+$/);if(!v)return;
  const want=clockFmt(clock.run/1000);
  if(v.textContent.trim()!==want)v.textContent=want;
}
...
function paintAge(){
  ...
  window.__FH_DIFFICULTY__={age,year,offset:clock.offset,baseAge:clock.baseAge,seconds:Math.round(clock.run/1000),armed:clock.armed};
}
function resetClockIfNewGame(){
  const b=document.querySelector('.crowd-board');
  if(b&&clock.lastBoard&&b!==clock.lastBoard){clock.baseAge=num(ageEl()?.textContent);clock.offset=0;clock.bank=0;clock.run=0;clock.lastRendered=null;clock.lastTick=performance.now()}
  if(b)clock.lastBoard=b;
}
// Pausa er det du faktisk ser, ikkje ei klasse i DOM-en. Meny-vaktposten er
// sett `hidden`, men arket her gav `.fh-modal` ein `display`, og ein
// forfattarregel slår `[hidden]` i nettlesaren sitt eige ark — så vaktposten
// var i praksis synleg, og blei han ståande att, stod både klokka og aldringa
// stille resten av spelet: «han eldst medan du leitar» gjorde ingenting.
function overlayShown(){
  for(const el of document.querySelectorAll('.gameover-card,.fh-modal,.fhm-overlay'))
    if(el.getClientRects().length)return true;
  return false;
}
function gameActive(){return !!document.querySelector('.crowd-board')&&!overlayShown()&&document.visibilityState!=='hidden'}
...
const TAP_SLOP=8;
const gesture={active:false,ids:new Set(),x:0,y:0,moved:false,multi:false,beforeAge:0,beforeRound:''};
function judgeTap(beforeAge,beforeRound){setTimeout(()=>{
  syncBaseAge();
  if(!gameActive())return;
  if(clock.baseAge>beforeAge)return;          // oppstraums la på eit år: funn
  if(roundToken()!==beforeRound)return;       // nytt brett: funn
  if(scored())return;                         // «+1 ÅR» står på skjermen: funn
  flashPenalty(1);
},700)}
function wireMistakes(){
  ...
  b.addEventListener('pointerdown',e=>{
    ...
    gesture.beforeAge=clock.baseAge;gesture.beforeRound=roundToken();
  },{passive:true});
  ...
  const end=(e,cancelled)=>{
    ...
    if(tap)judgeTap(gesture.beforeAge,gesture.beforeRound);
  };
...
function gameover(){
  ...
  const stats=[...card.querySelectorAll('.final-stats>div')],streak=num(stats.find(x=>/STREAK/i.test(x.textContent||''))?.querySelector('strong')?.textContent);
  const seconds=Math.round(clock.run/1000);
  const scoreTile=stats.find(x=>/SCORE|KLOKKA/i.test(x.textContent||''));
  if(scoreTile){const l=scoreTile.querySelector('span'),v=scoreTile.querySelector('strong');if(l)l.textContent='TID';if(v)v.textContent=clockFmt(seconds)}
  if(!age&&!seconds)return;card.dataset.lbState='saving';let box=document.createElement('section');box.className='fh-result-rank';box.innerHTML='<span>VERDSLISTE</span><strong>…</strong>';card.querySelector('.final-stats')?.insertAdjacentElement('afterend',box);
  board('POST',{age,score:seconds,streak}).then(data=>{card.dataset.lbState='saved';const rank=data.player?.rank;if(box.isConnected)box.innerHTML=`<span>VERDSLISTE</span><strong>${rank?'#'+fmt.format(rank):'—'}</strong><button type="button">SJÅ LISTA</button>`;box.querySelector('button')?.addEventListener('click',openBoard)}).catch(()=>{card.dataset.lbState='error';if(box.isConnected)box.innerHTML='<span>VERDSLISTE</span><strong>—</strong>'});
}
...
.first-instruction{display:none!important}.score-pop strong{display:none!important}.masthead{position:relative}.fh-top{position:absolute;right:0;bottom:-18px;width:30px;height:30px;display:grid;place-items:center;border:0;background:transparent;color:#6e695e;padding:6px;z-index:9}.fh-top svg{width:18px;height:18px;fill:none;stroke:currentColor;stroke-width:1.6;stroke-linecap:round;stroke-linejoin:round}.fh-top:focus-visible{outline:1px solid #81796d;outline-offset:1px}
...
function run(){style();button();tagLayout();wireMistakes();syncBaseAge();paintAge();paintClock();gameover()}
