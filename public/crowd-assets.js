(()=>{
  ...
  function haraldBoost(a){if(a<=2)return 1.42;if(a<=12)return 1.18;if(a<=30)return 1.08;return 1.04}
  ...
  function remember(img){if(!img.dataset.fhOriginal)img.dataset.fhOriginal=img.dataset.originalSprite||img.getAttribute('src')||''}
  ...
  function distribute(board,y){
    ...
    const ordered=figures.map((img,i)=>({img,i,k:hash(`${era}|${img.dataset.originalSprite||img.src}|${i}`)})).sort((a,b)=>a.k-b.k);
    const vp=p.map(a=>({a,k:hash(`${era}|verified|${a.id}`)})).sort((a,b)=>a.k-b.k);
    ordered.forEach((f,j)=>{if(vp.length&&j<Math.min(vp.length,Math.ceil(ordered.length*.45)))assignVerified(f.img,vp[j%vp.length].a);else if(FALLBACK.length)assignFallback(f.img,FALLBACK[hash(`${era}|fallback|${j}`)%FALLBACK.length],j);});
  }
  ...
  function style(){if(document.getElementById('fh-quality-crowd-style'))return;const s=document.createElement('style');s.id='fh-quality-crowd-style';s.textContent=`
    ...
    .crowd-board img.fh-quality-crowd{height:var(--fh-crowd-body-h,8.45%)!important;width:auto!important;max-width:17%!important}
    .crowd-board img.harald-target{height:var(--fh-harald-h,9.8%)!important;width:auto!important;max-width:17%!important;object-fit:contain!important;scale:var(--fh-render-scale,1)!important;z-index:4!important}
    .crowd-board img.crowd-figure:not(.harald-target){scale:var(--fh-render-scale,1)!important}
    @media(max-width:700px){.crowd-board{--fh-crowd-body-h:8.25%;--fh-harald-h:9.65%}.crowd-board img.fh-quality-crowd{max-width:18%!important}}
